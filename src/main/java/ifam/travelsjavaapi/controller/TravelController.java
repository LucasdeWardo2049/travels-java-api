    ...
    public ResponseEntity<Travel> create(@RequestBody JSONObject travel) {
        try {
            if(travelService.isJSONValid(travel.toString())) {
                ...
                if(travelService.isStartDateGreatherThanEndDate(travelCreated)){
        ...
        try {
            if(travelService.isJSONValid(travel.toString())) {
                ...
                if(travelToUpdate == null){
                    ...
                    Travel travelUpdate = travelService.update(travelToUpdate, travel);
                    return ResponseEntity.ok(travelUpdate);
                }
            }else {
                return ResponseEntity.badRequest().body(null);
            }
